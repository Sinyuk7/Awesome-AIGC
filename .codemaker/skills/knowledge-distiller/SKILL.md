...
description: |
  知识蒸馏器 - 将指定目录下的多源材料（文档、笔记、raw data）蒸馏为单一结构化认知型知识文档。
  这是一个交互式对话型 Skill，会像 Agent 一样逐步与用户确认蒸馏范围、焦点、模块选择等细节。
  当用户提到"蒸馏"、"distill"、"整理知识"、"合并文档"、"提炼知识"、"归纳总结目录内容"时，应该使用这个 Skill。
  也适用于用户指定一个包含多个文件的目录，希望将其中的知识整合成一份结构化文档的场景。
---
